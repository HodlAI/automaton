...
import type { HodlAIClient, GitStatus, GitLogEntry } from "../types.js";
...
export async function gitStatus(
  ...
  const result = await hodlai.exec(
...
export async function gitCommit(
  ...
  if (addAll) {
    await hodlai.exec(`cd ${repoPath} && git add -A`, 10000);
  }
...
export async function gitBranch(
  ...
  const result = await hodlai.exec(cmd, 10000);
