...
describe("executeFundingStrategies", () => {
  ...
  beforeEach(() => {
    ...
    hodlai.creditsCents = 5; // low balance
  });
  ...
  it("respects per-tier cooldown on repeated calls", async () => {
    ...
    const first = await executeFundingStrategies("dead", identity, config, db, hodlai);
    expect(first.length).toBe(1);

    // Immediate second dead-tier call should be suppressed (2h cooldown)
    const second = await executeFundingStrategies("dead", identity, config, db, hodlai);
