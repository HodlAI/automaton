 ...
 *
 * Canonical format: HodlAI:send:{to_lowercase}:{keccak256(toBytes(content))}:{signed_at_iso}
 */
export async function signSendPayload(
  ...
  const canonical = `HodlAI:send:${to.toLowerCase()}:${contentHash}:${signedAt}`;
 ...
 * Sign a poll payload.
 *
 * Canonical format: HodlAI:poll:{address_lowercase}:{timestamp_iso}
 */
export async function signPollPayload(
  account: PrivateKeyAccount,
): Promise<{ address: string; signature: string; timestamp: string }> {
  const timestamp = new Date().toISOString();
  const canonical = `HodlAI:poll:${account.address.toLowerCase()}:${timestamp}`;
  const signature = await account.signMessage({ message: canonical });
