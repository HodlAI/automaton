...
export async function propagateConstitution(
  ...
  await hodlai.writeFile("/root/.automaton/constitution.md", constitution);

  // Write hash file for the child to verify against
  await hodlai.writeFile("/root/.automaton/constitution.sha256", hash);
  ...
  try {
    await hodlai.exec("chmod 444 /root/.automaton/constitution.md", 5000);
  } catch {
    // Non-critical
  }
}
...
export async function verifyConstitution(
  ...
  try {
    ...
    const childConstitution = await hodlai.readFile("/root/.automaton/constitution.md");
