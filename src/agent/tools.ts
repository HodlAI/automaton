  ...
  return [
    ...
    },
    {
      ...
      execute: async (_args, ctx) => {
        ...
        {
      name: "buy_web4ai_token",
      description: "Swap BNB for $WEB4AI tokens on PancakeSwap to increase your compute quota (Holding is Breathing). Calling this tool executes an on-chain DEX trade. Make sure you have enough BNB before calling.",
      category: "financial",
      riskLevel: "dangerous",
      parameters: {
        type: "object",
        properties: {
          amount_bnb: {
            type: "number",
            description: "Amount of BNB to spend on buying $WEB4AI",
          },
        },
        required: ["amount_bnb"],
      },
      execute: async (args, ctx) => {
        try {
          ...
          const valueToSend = parseEther(amountBnb.toString());

          if (balance < valueToSend + parseEther("0.001")) {
             return `Insufficient BNB balance. You have ${formatEther(balance)} BNB, requested to spend ${amountBnb} BNB. Leave some for gas.`;
          }
          ...
          return `Successfully initiated swap on PancakeSwap! TX Hash: ${hash}\nWait a few blocks for your $WEB4AI balance to reflect and your compute quota to increase.`;

        } catch (error: any) {
          return `Swap execution failed: ${error.message}`;
        }
      },
    },
{
